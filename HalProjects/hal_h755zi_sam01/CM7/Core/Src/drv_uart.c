...
void taskUartDriverInput(void)
{
	...
	if (u8s_RxActiveState == OFF) {
		/* UART受信(割り込み)を開始 */
		if (HAL_UART_Receive_IT(&huart3, (uint8_t *)&u8s_HalRxBuffer[0], UART_RX_BLOCK_SIZE) != HAL_OK) {
			/* Reception Error */
			Error_Handler();
		}
...
void taskUartDriverOutput(void)
{
	...
	if (u8s_TxActiveState == OFF) {
		...
		if (u16_TxSize > 0) {
			/* UART送信(割り込み)を開始 */
			if (HAL_UART_Transmit_IT(&huart3, (uint8_t *)&u8s_HalTxBuffer[0], u16_TxSize) != HAL_OK) {
				/* Transmission Error */
				Error_Handler();
			}
